    ...
    if adversarial_learning['enabled']:
        for i in range(2, adversarial_learning['iterations']+1):
            ...
            if i > 1:
                ...
                infer_kwargs['infer']['crate']['max'] = np.inf
                solver_kwargs['use_success_threshold'] = True

            infer_kwargs['solver_path'] = None

            solver_kwargs['generator_path'] = os.path.join(log_dir, 'generator', 'model', 'latest_model')
            ...
            generator_kwargs['solver_path'] = os.path.join('runs', f'{experiment_name}_{experiment_idx}_log', 'solver', 'model', 'latest_model')
