...
class State:
    ...
    def clone(self):
        ...
        for t in self.traps:
            clone.traps.append({"x":t["x"], "y":t["y"]})
    ...
    def checkTrapLocation(self, x, y):
        for t in self.traps:
            if t["x"] == x and t["y"] == y:
                return t
        return None
    ...
    def checkMovableLocation(self, x, y):
        return not self.checkOutside(x, y) and not self.solid[y][x] and self.checkCrateLocation(x,y) is None and self.checkTrapLocation(x,y) is None

    def checkWin(self):
        return self.player["x"] == self.targets[0]["x"] and self.player["y"] == self.targets[0]["y"]

    def getHeuristic(self):
        target = self.targets[0]
        return abs(target["x"] - self.player["x"]) + abs(target["y"] - self.player["y"])
