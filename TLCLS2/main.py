...
def train(args, wandb_session):
    ...
    elif 'pull' in args.task:
        fix = args.task[5]
        source_env_name = 'Curriculum-Sokoban-PushAndPull-v1'
        target_env_name = 'Curriculum-Sokoban-PushAndPull-v1'
    else:
        fix = args.task[5]
        source_env_name = 'Curriculum-Sokoban-v2'
    ...
    source_task_map = os.path.join(args.map_file, args.map_size, str(source_task))
    target_task_map = os.path.join(args.map_file, args.map_size, str(target_task))
    ...
    train_the_agent(envs, args.num_envs, source_env_name, Variable, state_shape, actor_critic, optimizer, rollout, data_path=None, args=args, wandb_session=wandb_session) #train and save the model;
    ...
    train_the_agent(envs, args.num_envs, target_env_name, Variable, state_shape, actor_critic, optimizer, rollout, data_path=target_task_map, args=args, wandb_session=wandb_session) #train and save the model;

    #save the model
    torch.save(actor_critic.state_dict(), os.path.join(wandb_session.dir, 'model.pkl'))

if __name__ == "__main__":
    description = 'TLCLS'
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument('--num_steps', type=int, default=1e6)
    parser.add_argument('--task', type=str, default='s1t2k2')
